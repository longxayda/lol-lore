...
def get_main() -> T.List[T.Dict[str, str]]:
    source = modules.scraping.utilities.get_content("home")
    json_path = JSON_DIRECTORY_PATH + "/champs.json"
    html_path = HTML_DIRECTORY_PATH + "/champs.html"
    soup = bs4.BeautifulSoup(source, "html.parser")
    gen: T.Generator[T.Union[bs4.NavigableString, bs4.Tag, None], None, None] = (
    ...
    for li in gen:
        ...
        if (
            a is not None
            and a.has_attr("href")
            and re.match(r"\/en_US\/champion\/[a-zA-Z0-9_-]+\/", a["href"])
        ):
            name = a.find("h1").string
            region = a.find("span").string
            img_ref = a.find_all("div", {"data-am-url": True}, recursive=True)[0][
                "data-am-url"
            ]
            output.append(
                {
...
def get_champ(data: T.Dict) -> T.List[T.Dict[str, str]]:
    ...
    source = modules.scraping.utilities.get_content("champion", search_name)
    soup = bs4.BeautifulSoup(source, "html.parser")
    ...
    champ_race = modules.scraping.utilities.recursive_find(
        soup, [["div", {"class": re.compile(patterns["race"])}], ["h6"]]
    )
    champ_quote = modules.scraping.utilities.recursive_find(
        soup, [["li", {"class": re.compile(patterns["quote"])}], ["p"]]
    )
    champ_type = modules.scraping.utilities.recursive_find(
        soup, [["div", {"class": re.compile(patterns["type"])}], ["h6"]]
    )
    champ_name = modules.scraping.utilities.recursive_find(
        soup, [["div", {"class": re.compile(patterns["name"])}], ["span"]]
    )
    champ_region = modules.scraping.utilities.recursive_find(
        soup, [["div", {"class": re.compile(patterns["region"])}], ["h6"], ["span"]]
    )
    champ_biography_text = modules.scraping.utilities.recursive_find(
        soup, [["div", {"class": re.compile(patterns["biography_text"])}], ["p"]]
    )
...
def get_lore(data: T.Dict[str, str]) -> T.Dict[str, str]:
    ...
    source = modules.scraping.utilities.get_content("story", search_name)
    soup = bs4.BeautifulSoup(source, "html.parser")
...
def run():
    ...
    if len(sys.argv) > 1:
        ...
        for champ_name in champ_names:
            ...
            for champ in champs:
                if (
                    champ["name"].lower() == champ_name
                    or champ["ref_name"] == champ_name.lower()
                ):
        ...
        while len(champs) and retries:
            validation_result = modules.scraping.utilities.validate()
